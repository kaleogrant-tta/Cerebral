...
def check_weeks_aligned(dash):
    ...
    for t in sorted(have):
        ...
        else: report(f"latest week {t}", WARN, f"{y}-W{w:02d} - {lag} weeks behind sales")
...
def check_fact_line_basics(db):
    ...
    report("fact_line: cannabis lines have a brand", PASS if nb == 0 else WARN,
           f"{nb:,} paid non-accessory lines with blank brand")
...
def check_inventory_fresh(db):
    ...
    if "fact_inventory_week" in tables(db):
        ...
        if qc:
            n = q1(db, " + ".join(f'(select count(*) from fact_inventory_week where "{c}" < 0)' for c in qc)
                     if len(qc) > 1 else f'select count(*) from fact_inventory_week where "{qc[0]}" < 0')
...
def check_dim_event(db):
    ...
    report("dim_event: shared-cost realloc matches export", PASS if d == 0 else WARN,
           f"{d} rows where realloc_delta != 0 (re-split disagrees with Airtable export)")
    ns = q1(db, """select count(*) from (select airtable_record_id, n_stores, count(*) c from dim_event
                   group by 1,2 having c > n_stores)""")
    report("dim_event: rows per event <= n_stores", PASS if ns == 0 else FAIL, f"{ns} events over")
...
def check_dash_inventory_current(dash, db):
    ...
    report("dash_inventory qoh = fact_inventory (same snapshot)", PASS if diff <= 0.02 else WARN,
           f"dash {d_tot:,.0f} vs fact {f_tot:,.0f} ({diff:.1%}){'' if 'sellable' in fcols else ' - no sellable col, compared all rooms'}")

# ----------------------------------------------------- business invariants
def check_alias_applied(dash):
    leak = q1(dash, """select count(*) from dash_brand_week w
                       join dash_brand_alias a on w.brand = a.alias and a.alias <> a.canonical""")
    report("brand aliases applied in dash_brand_week", PASS if leak == 0 else FAIL, f"{leak} rows still on a raw alias")
    if "dash_vm_brand_week" in tables(dash):
        leak = q1(dash, """select count(*) from dash_vm_brand_week w
                           join dash_brand_alias a on w.brand = a.alias and a.alias <> a.canonical""")
        report("brand aliases applied in dash_vm_brand_week", PASS if leak == 0 else FAIL, f"{leak} rows")

def check_event_tracker(dash):
    t = "dash_event_tracker"
    check_no_dupes(dash, t, ["airtable_record_id"])
...
def check_stock_week(dash):
    ...
    z = q1(dash, f"select count(*) from {t} where brand_stockout and products_stocked_out = 0")
    report("stock-week: brand_stockout implies a stocked-out product", PASS if z == 0 else FAIL, f"{z} rows")
...
def main():
    ...
    print("-- invariants")
    run("alias", check_alias_applied, dash); run("event tracker", check_event_tracker, dash)
